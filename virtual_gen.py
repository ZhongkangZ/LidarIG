...
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=50, dist_thresh=3000, num_camera=6, intrinsics=None, transforms=None):
    ...
    # avoid matching across instances ，避免实例ID与其他数值混淆
    real_points[:, -1] *= 1e4 
...
if __name__ == '__main__':
    ...
    DATA_PATH = '/root/autodl-tmp/nuScenes'
    
    if not os.path.isdir(DATA_PATH + '/samples/LIDAR_TOP_VIRTUAL'):
        os.mkdir(DATA_PATH + '/samples/LIDAR_TOP_VIRTUAL')

    if not os.path.isdir(DATA_PATH + '/sweeps/LIDAR_TOP_VIRTUAL'):
        os.mkdir(DATA_PATH + '/sweeps/LIDAR_TOP_VIRTUAL')
